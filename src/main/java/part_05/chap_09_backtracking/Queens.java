...
public class Queens {
    ...
    final boolean
            available = true;                                             // * added only for readability of the code
    final int
            squares = 4,                                                        // * length of the board sid
            norm = squares - 1;
    int
            howMany = 0;                                                            // ? how many queens on the board already?
    int[]
    ...
    void PutQueen(int row) {                                                    // * the method operates in a defined row
        ...
        for (int col = 0; col < squares; col++)                                 //
            ...
            )
            {
                ...
                if (row < squares - 1)
                    PutQueen(row + 1);                                     // ! is the row the last one: recursive call to another row

                else
                    PrintBoard(System.out);                                    // * upon hitting the end - print the results
