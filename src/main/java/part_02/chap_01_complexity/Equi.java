...
public class Equi {
	...
	static void equal_01(int[] input) {
		...
		if (tableLength-- > 0) {
			...
			for (int i = 0; i < tableLength; i++) a += input[i];

			for (int j = tableLength; j > 0; j--) {
				b += input[j];
				a -= input[j - 1];

				if (a == b) System.out.println("solution found for the index: " + j);
			}
		}
		else System.out.println("table too short for a proper solution");
	}
	...
	static void equal_02(int[] input){
		...
		if (tableLength > 0){
			...
			for (int j = 1 ; j < tableLength; j++){
				b += input[j - 1];
				c = input[j];
				if (a - (b + c) == b) System.out.println("solution for index : " + j);
			}
		}
	}
