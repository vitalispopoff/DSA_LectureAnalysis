...
public class Student {
    ...
    public int compareStudents(Student student) {
        ...
        if (student == null)
            return 1;         // ? a mistake? shouldn't return 1 ?

        else {
            int
                result = lastName.compareToIgnoreCase(student.lastName);
    ...
    public boolean isInList() {
        ...
        for (Student cache = studentHead; cache != null; cache = cache.next)

            if (compareStudents(cache) == 0)
                return true;
    ...
    public static Student addToList(String lastName, String firstName) {
        ...
        Student
            ...
            cache = Student.findNearest(input);
    ...
    public void unbindGrade(Grade grade, Lecture lecture) {
        ...
        if (lecture.gradeHead == grade) {
            ...
            if (grade.nextStudentGrade != null)
                grade.nextStudentGrade.prevStudentGrade = null;
        ...
        if (lecture.gradeTail == grade) {
            ...
            if (grade.prevStudentGrade != null)
                grade.prevStudentGrade.nextStudentGrade = null;
    ...
    public void verifyGradeReferencing(Grade grade) {
        ...
        if (grade.prevStudentGrade == null)
            for (; head.gradeHead != grade; head = head.next) ;

        if (grade.nextStudentGrade == null)
            for (; tail.gradeTail != grade; tail = tail.next) ;
    }
