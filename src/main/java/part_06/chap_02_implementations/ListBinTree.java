...
//  --------------------------------------------------------------------------------------------------------------------
    ...
    ListBinTree goToBranch(int destination) {
        ...
        ListBinTree
                cache = this;
        String
                path = Integer.toBinaryString(destination);
        ...
        for (int i = 1; i < path.length(); i++) {
            ...
            boolean
                    turnRight = path.charAt(i) == '1';

            if (cache != null)
                cache = turnRight ? cache.rightBranch : cache.leftBranch;
    ...
    static int findLowestLeafLocation(ListBinTree localRoot) {
        ...
        if (localRoot == null)
            ...
            for (; cache != null; ) {
                ...
                String
                        path = Integer.toBinaryString(i);

                for (int j = 1; j < path.length(); j++) {

                    cache = path.charAt(j) == '1'
                            ? cache.rightBranch
                            : cache.leftBranch;
                }
            }
    ...
    ListBinTree findLowerLeaf() {
        ...
        boolean
                ...
                right = rightBranch == null;
                                                        // we go left-to-right on the branches
        if (left)                                       // * left is a leaf (forget the right branch then)
            return leftBranch;

        else if (!left & right)                         // * left => branch (not leaf), right => leaf
            return rightBranch;

        else
            return this;                               // * branch is a root
    }

    static ListBinTree addToTail(String name) {

        int
                location = findLowestLeafLocation(root);
        ListBinTree
